...
class CriminalManagementSystem:
    ...
    def add_user(self, username, password, role, max_retries=5, retry_delay=1):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"IntegrityError: User '{username}' already exists. Skipping creation.")
                return False
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        if st.session_state.role == 'admin':
            admin_view(cms)
        elif st.session_state.role == 'user':  # Add an explicit check for 'user'
            user_view(cms)
        else:
